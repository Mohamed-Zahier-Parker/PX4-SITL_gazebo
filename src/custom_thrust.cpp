...
#include "custom_thrust.h"
#include <ignition/math.hh>

namespace gazebo {

Customthrust::~Customthrust() {
  updateConnection_->~Connection();
  use_pid_ = false;
}

void Customthrust::InitializeParams() {}

void Customthrust::Publish() {
...
void Customthrust::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  model_ = _model;
  ...
  //Time constant for motor delay
  this->tau_mot=0;
  if (_sdf->HasElement("tau_mot"))
    this->tau_mot = _sdf->Get<double>("tau_mot");

  this->in_scale=0;
  if (_sdf->HasElement("in_scale"))
    this->in_scale = _sdf->Get<double>("in_scale");

  this->thrust_Intergrator=0; //set intial thrust to zero
  this->disp_count=0;

  if (_sdf->HasElement("baselinkName"))
    base_link_name_ = _sdf->GetElement("baselinkName")->Get<std::string>();
  else
    gzerr << "[gazebo_motor_model] Please specify a baselinkName.\n";
  baselink_ = model_->GetLink(base_link_name_);
  if (baselink_ == NULL)
    gzthrow("[gazebo_motor_model] Couldn't find specified baselink \"" << base_link_name_ << "\".");
  ...
  motor_sub_ = node_handle_->Subscribe<mav_msgs::msgs::MotorSpeed>("~/" + model_->GetName() + motor_test_sub_topic_, &Customthrust::testProto, this);
  */
  ...
  updateConnection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&Customthrust::OnUpdate, this, _1));

  command_sub_ = node_handle_->Subscribe<mav_msgs::msgs::CommandMotorSpeed>("~/" + model_->GetName() + command_sub_topic_, &Customthrust::VelocityCallback, this);
  //std::cout << "[gazebo_motor_model]: Subscribe to gz topic: "<< motor_failure_sub_topic_ << std::endl;
  motor_failure_sub_ = node_handle_->Subscribe<msgs::Int>(motor_failure_sub_topic_, &Customthrust::MotorFailureCallback, this);
  // FIXME: Commented out to prevent warnings about queue limit reached.
  //motor_velocity_pub_ = node_handle_->Advertise<std_msgs::msgs::Float>("~/" + model_->GetName() + motor_speed_pub_topic_, 1);
  wind_sub_ = node_handle_->Subscribe("~/" + wind_sub_topic_, &Customthrust::WindVelocityCallback, this);
...
// Protobuf test
/*
void Customthrust::testProto(MotorSpeedPtr &msg) {
...
void Customthrust::OnUpdate(const common::UpdateInfo& _info) {
  ...
  disp_count++;
  // if(disp_count%200==0){
  //   printf("!!!Thrust_plugin Values!!!\n");
  //   printf("Input: %f ; Thrust: %f \n",joint_->GetVelocity(0),this->thrust_Intergrator);
  //   printf("Actuator topic testor; da = %f ; de= %f; df= %f; dR= %f \n",dA_defl_,dE_defl_,dF_defl_,dR_defl_);
  // }
...
void Customthrust::VelocityCallback(CommandMotorSpeedPtr &rot_velocities) {
...
void Customthrust::MotorFailureCallback(const boost::shared_ptr<const msgs::Int> &fail_msg) {
...
void Customthrust::UpdateMotorFail() {
...
void Customthrust::WindVelocityCallback(WindPtr& msg) {
  wind_vel_ = ignition::math::Vector3d(msg->velocity().x(),
            msg->velocity().y(),
            msg->velocity().z());
}

GZ_REGISTER_MODEL_PLUGIN(Customthrust);
}
